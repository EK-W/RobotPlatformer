...
public class Vector2D {
	...
	public Vector2D minVector(Vector2D with){
		return new Vector2D(Math.min(this.x, with.x), Math.min(this.y, with.y));
	}
	
	public Vector2D maxVector(Vector2D with){
		return new Vector2D(Math.max(this.x, with.x), Math.max(this.y, with.y));
	}
	/**
	 * 
